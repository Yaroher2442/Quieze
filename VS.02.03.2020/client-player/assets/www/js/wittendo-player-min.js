...
angular.module("app").controller("AppCtrl", ["$scope", "GameService", "RegistrationService", "GameEvents", "$rootScope", function(a, b, c, e, r) {
...
}]).controller("QuestionCtrl", ["$scope", "$timeout", "GameService", "$rootScope", function(a, b, c, d) {
	a.Math = window.Math;
	...
	a.$watch(function() {
		return d.expressQuestionShow;
	}, function(newVal) {		
		a.expressQuestionShow = newVal;
	});
...
angular.module("app").factory("GameService", ["$rootScope", "$http", "server", "$window", "$location", "$route", "$timeout", function(a, b, c, e, f, h, m) {
	...
	function q() {
		...
		if (g.question.type == "express" && d.initialSeconds == d.remainSeconds && d.selected == -1) {
			to = 4000;
			showExpressQuestion();
			m(hideExpressQuestion, 3000);
		}
	...
	function showExpressQuestion() {
		...
		a.expressQuestionShow = true;		
	}

	function hideExpressQuestion() {
		$("#questionModal").modal("hide");
		a.expressQuestionShow = false;		
	}
	...
	return {
		...
		getExpressQuestionState: function() {
